...
#["lexic_metropolis","lexic_glauber","random_metropolis","random_glauber"]
for alg in ["lexic_metropolis","lexic_glauber"]:
	L=64
	name = "charge"
	f = lambda x, a, b:a*np.exp(-x/b)

	data = np.loadtxt(f"output/autocorrelation/{name}_{alg}.csv",skiprows=1,delimiter=",")[:-1].transpose()

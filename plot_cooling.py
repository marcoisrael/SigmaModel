...
for alg in algs:
	...
	for i in indexes:
		...
		if i in [4,6,8]:
			ax1.errorbar(data[0], data[ob], data[ob+1],color=colors[i], ls="",marker="o",markersize=5,label=f"$\\tau_{{\\mathrm{{cool}}}}={i}$")
	...
	ax2.text(0.99,0.99,r"$\frac{\chi^2}{\mathrm{dof}}=$"+str(xfit.chisq_by_dof),fontsize=16,ha='right', va='top',transform=ax2.transAxes)
